...
FUNCTIONS = [
    ...
    {
        "name": "NORM.DIST", "category": "Statistical", "difficulty": "Advanced",
        "syntax": "=NORM.DIST(x, mean, standard_dev, cumulative)",
        "description": "Returns the normal distribution probability for a given value. With cumulative=TRUE gives P(X ≤ x); with FALSE gives the probability density at x.",
        "use_case": "Risk: Estimate the probability that a portfolio loss exceeds a threshold under a normal returns assumption; compute VaR confidence levels in a credit risk model.",
        "tip": "⚠️ cumulative=TRUE (CDF) is almost always what you need in finance. cumulative=FALSE returns density, which is not a probability and is rarely useful directly.",
        "applications": ["Valuations", "FP&A"],
    },
    {
        "name": "NORM.INV", "category": "Statistical", "difficulty": "Advanced",
        "syntax": "=NORM.INV(probability, mean, standard_dev)",
        "description": "Returns the value (quantile) at a given cumulative probability for a normal distribution — the inverse of NORM.DIST.",
        "use_case": "Risk: Calculate the 99th-percentile loss threshold for VaR; find the revenue level that will be exceeded with 90% probability in a Monte Carlo stress scenario.",
        "tip": "⚠️ Probability must be between 0 and 1 (exclusive). Use =NORM.INV(1-confidence, mean, sd) to get the lower tail cutoff for loss modelling.",
        "applications": ["Valuations", "FP&A"],
    },
    {
        "name": "NORM.S.DIST", "category": "Statistical", "difficulty": "Intermediate",
        "syntax": "=NORM.S.DIST(z, cumulative)",
        "description": "Returns the standard normal distribution (mean=0, sd=1) probability for a z-score. Equivalent to NORM.DIST(z, 0, 1, cumulative).",
        "use_case": "Valuations: Used inside Black-Scholes option pricing — N(d1) and N(d2) are NORM.S.DIST calls; compute p-values from z-statistics in hypothesis tests.",
        "tip": "⚠️ Always pre-compute your z-score with STANDARDIZE() before passing to NORM.S.DIST to keep formulas readable and auditable.",
        "applications": ["Valuations"],
    },
    {
        "name": "NORM.S.INV", "category": "Statistical", "difficulty": "Intermediate",
        "syntax": "=NORM.S.INV(probability)",
        "description": "Returns the z-score corresponding to a given cumulative probability on the standard normal distribution.",
        "use_case": "Risk: Build a parametric VaR model — multiply =NORM.S.INV(0.99) × portfolio_sigma to get the 99% VaR; construct confidence interval bounds.",
        "tip": "⚠️ =NORM.S.INV(0.975) = 1.96 (the classic 95% two-tailed z-score). For one-tailed 95% use 0.95, not 0.975.",
        "applications": ["Valuations", "FP&A"],
    },
    {
        "name": "STANDARDIZE", "category": "Statistical", "difficulty": "Intermediate",
        "syntax": "=STANDARDIZE(x, mean, standard_dev)",
        "description": "Converts a raw value to a z-score: (x − mean) / standard_dev.",
        "use_case": "Risk: Normalise returns across assets with different scales before computing correlation or running a z-test; flag revenue outliers more than 2 standard deviations from the mean.",
        "tip": "⚠️ Returns #DIV/0! if standard_dev = 0. Guard with =IF(STDEV.S(range)=0, 0, STANDARDIZE(...)).",
        "applications": ["Valuations", "FP&A"],
    },
    {
        "name": "BINOM.DIST", "category": "Statistical", "difficulty": "Advanced",
        "syntax": "=BINOM.DIST(number_s, trials, probability_s, cumulative)",
        "description": "Returns the probability of exactly (or at most) k successes in n independent Bernoulli trials with a fixed probability p.",
        "use_case": "Risk: Model the probability of k defaults in a loan portfolio of n borrowers assuming independent default probabilities; price binary credit derivatives.",
        "tip": "⚠️ cumulative=FALSE gives the exact probability of exactly k successes; TRUE gives P(X ≤ k). For P(X ≥ k) use 1 − BINOM.DIST(k−1, n, p, TRUE).",
        "applications": ["Valuations"],
    },
    {
        "name": "POISSON.DIST", "category": "Statistical", "difficulty": "Advanced",
        "syntax": "=POISSON.DIST(x, mean, cumulative)",
        "description": "Returns the Poisson distribution probability for x events when the expected average rate is known.",
        "use_case": "Risk: Model the probability of a given number of operational loss events in a quarter; estimate claim frequency in insurance pricing or trade error rates in operations.",
        "tip": "⚠️ The Poisson model assumes events are independent and the rate is constant — validate these assumptions before using it in regulatory models.",
        "applications": ["Valuations"],
    },
    {
        "name": "EXPON.DIST", "category": "Statistical", "difficulty": "Advanced",
        "syntax": "=EXPON.DIST(x, lambda, cumulative)",
        "description": "Returns the exponential distribution probability — models the time between independent events occurring at a constant average rate (lambda).",
        "use_case": "Risk: Model time-to-default or time between credit events; estimate the probability that a system failure occurs within a given time window in operational risk.",
        "tip": "⚠️ Lambda is the rate (events per unit time), not the mean wait time. If mean wait = 2 years, lambda = 0.5. Confusing the two is a common modelling error.",
        "applications": ["Valuations"],
    },
    {
        "name": "CONFIDENCE.NORM", "category": "Statistical", "difficulty": "Intermediate",
        "syntax": "=CONFIDENCE.NORM(alpha, standard_dev, size)",
        "description": "Returns the margin of error (half-width) of a confidence interval for a population mean using the normal distribution.",
        "use_case": "FP&A: Report revenue forecasts with explicit confidence intervals — e.g. 'Q3 revenue: ₹150Cr ± CONFIDENCE.NORM(0.05, sigma, n)' to quantify forecast uncertainty.",
        "tip": "⚠️ Alpha is the significance level, not the confidence level — for 95% confidence use alpha=0.05. Add and subtract the result from the sample mean to build the full interval.",
        "applications": ["FP&A", "Valuations"],
    },
...
st.html("""
<div style="background:linear-gradient(135deg,#7B1FA2 0%,#4527A0 50%,#1565C0 100%);
     border-radius:14px;padding:24px 32px;margin-bottom:22px;
     border:1.5px solid rgba(232,184,75,0.55);
     box-shadow:0 6px 28px rgba(69,39,160,0.28);min-height:120px;">

  <div style="font-size:0.65rem;font-weight:800;letter-spacing:2px;
       color:#E8B84B;text-transform:uppercase;margin-bottom:6px;">
    &#9679; NOW ENROLLING &mdash; COHORT OPENS MID-JULY 2026
  </div>

  <div style="font-size:1.22rem;color:#FFFFFF;font-weight:700;
       margin-bottom:10px;line-height:1.3;">
     Live Finance Courses &mdash; The Mountain Path Academy
  </div>

  <div style="margin-bottom:12px;">
    <span style="background:rgba(232,184,75,0.2);border:1px solid rgba(232,184,75,0.5);
         border-radius:20px;padding:4px 14px;font-size:0.82rem;font-weight:700;
         color:#E8B84B;margin-right:10px;display:inline-block;margin-bottom:6px;">
      Applied Finance Through Excel
    </span>
    <span style="background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.3);
         border-radius:20px;padding:4px 14px;font-size:0.82rem;font-weight:700;
         color:#D4E1FF;display:inline-block;margin-bottom:6px;">
      Financial Analytics
    </span>
  </div>

  <div style="font-size:0.84rem;color:#A8BEDD;margin-bottom:16px;">
    Hands-on &bull; Practitioner-led &bull; Built for analysts who want to move from theory to the real deal.
  </div>

  <a href="https://themountainpathacademy.com" target="_blank"
     style="display:inline-block;background:linear-gradient(135deg,#C8962E 0%,#E8B84B 100%);
            color:#0D1B3E;font-weight:800;font-size:0.85rem;padding:10px 26px;
            border-radius:25px;text-decoration:none;letter-spacing:0.5px;
            box-shadow:0 4px 14px rgba(200,150,46,0.45);">
    Register Now &#8594;
  </a>
  <span style="font-size:0.7rem;color:#A8BEDD;margin-left:14px;vertical-align:middle;">
...
# ─── GITHUB DOWNLOAD BANNER ───────────────────────────────────────────────────
GITHUB_URL = "https://github.com/trichyravis/Mastering-Excel-for-Finance-The-Mountainpath-Academy/tree/main"
st.html(f"""
<div style="
    display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:16px;
    background: linear-gradient(135deg, #0D1B3E 0%, #162552 60%, #1E3A6E 100%);
...
">
  ...
  <div style="flex:1; min-width:220px;">
    <div style="font-size:0.68rem;font-weight:700;color:#C8962E;letter-spacing:1.2px;
         text-transform:uppercase;margin-bottom:6px;">
      Free Resource
    </div>
    <div style="font-size:1.05rem;font-weight:700;color:#FFFFFF;margin-bottom:4px;
         font-family:'Playfair Display',serif;">
      Mastering Excel for Finance
    </div>
    <div style="font-size:0.82rem;color:#B0BCDA;line-height:1.5;">
      Download the full Excel workbook — 11 sheets, 90+ functions with live examples,
      sample data &amp; formulas. Free on GitHub.
    </div>
  </div>
  ...
  <div style="flex-shrink:0;">
    <a href="{GITHUB_URL}" target="_blank" style="
        ...
        background:#C8962E; color:#FFFFFF; text-decoration:none;
        padding:12px 24px; border-radius:10px; font-weight:700;
        font-size:0.88rem; letter-spacing:0.3px;
        box-shadow:0 4px 14px rgba(200,150,46,0.40);
        transition:background 0.2s;
    ">
